...
Sub ShowUsageAndQuit()
    ...
    sUsage = sUsage & "WinEventLogText.vbs target file.csv ""logname(s)"" [/clear] [/v] [/dumphex] " & vbCrLf
    sUsage = sUsage & "WinEventLogText.vbs target file.csv /all [/clear] [/v] [/dumphex] " & vbCrLf
    ...
    sUsage = sUsage & "        WinEventLogText.vbs server1 output.csv ""application,system"" /clear" & vbCrLf
    sUsage = sUsage & "    Example: the following will dump all logs from server1 to output.csv:" & vbCrLf
    sUsage = sUsage & "        WinEventLogText.vbs server1 output.csv /all" & vbCrLf
